...
public class IntLongOpenHashMapTemplate extends
        AbstractTemplate<IntLongOpenHashMap> {

    /** Instantiates a new int long open hash map template. */
    protected IntLongOpenHashMapTemplate() {
        super(null, IntLongOpenHashMap.class, 1, ObjectType.MAP, false);
    }

    /** Computes the size; (key+value) counts as *1*. */
    @Override
    public final int getSpaceRequired(final PackerContext theContext,
            final IntLongOpenHashMap theValue) {
        return theValue.size();
    }

    /** {@inheritDoc} */
    @Override
    public IntLongOpenHashMap preCreate(final int theSize) {
        return new IntLongOpenHashMap(theSize);
    }

    /** {@inheritDoc} */
    @Override
    public IntLongOpenHashMap readData(final UnpackerContext theContext,
            final IntLongOpenHashMap thePreCreated, final int theSize)
            throws IOException {
    ...
    public void writeData(final PackerContext theContext, final int theSize,
            final IntLongOpenHashMap theValue) throws IOException {
        final Packer p = theContext.packer;
        for (final IntLongCursor k : theValue) {
            p.writeInt(k.key);
            p.writeLong(k.value);
        }
    }
